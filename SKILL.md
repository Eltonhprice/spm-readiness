...
**Input:** `spm-inputs/<client>/` — collector .txt exports per domain subfolder  
...
```bash
python -m scripts.html_deck --metrics spm-outputs/<client>/metrics.json --out spm-outputs/<client>
```
...
4. Save as `spm-inputs/<client>/<domain>/<table>.txt`
5. Run the two sidecars last: `_sidecar_spm_adoption.js` and `_sidecar_portfolio_health.js` → save to `spm-inputs/<client>/sidecars/`
